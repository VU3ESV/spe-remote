...
# orchestrator only uses FlexConfig as a type hint, so stub it out.
...
class FakeFlex:
    def __init__(self, serial, freq_mhz="14.074000", mode="USB"):
        self._serial = serial
        self.slice_state = {0: {"RF_frequency": freq_mhz, "mode": mode}}
        self.calls = []

    async def set_slice_freq(self, rx, freq):
        self.calls.append(("freq", rx, freq))
        self.slice_state.setdefault(rx, {})["RF_frequency"] = f"{freq:.6f}"

    async def set_slice_mode(self, rx, mode):
        self.calls.append(("mode", rx, mode))
...
def make(op_status="Stby", band="20m", freq="14.074000", oper_works=True,
         flex_none=False, mode="USB"):
    serial = FakeSerial(op_status, band, oper_works)
    flex = FakeFlex(serial, freq, mode)
    ctrl = FakeController(flex)
    if flex_none:
        async def _none():
            return None
        ctrl.connect = _none
    phases = []
    orch = TuneOrchestrator(serial, ctrl, Cfg(),
                            on_status=lambda p, m: phases.append((p, m)))
    return orch, serial, flex, phases
...
async def t1_sweep_operate_restore():
    ...
    orch, serial, flex, phases = make(op_status="Oper")
...
async def t2_sweep_already_stby():
    ...
    orch, serial, flex, phases = make(op_status="Stby")
    ok = await orch.tune_band("20m")
    ns = names(phases)
...
async def t3_radio_rules_override():
    ...
    orch, serial, flex, phases = make(op_status="Oper", freq="14.074000")
    ...
    check("t3 swept 20m freqs (not 40m)",
          all(14.0 < c[2] < 14.35 for c in flex.calls if c[0] == "freq"),
          str(flex.calls))
...
async def t4_auto_band():
    ...
    orch, serial, flex, phases = make(freq="14.200000")
    ...
    check("t4 swept 20m freqs",
          any(c[0] == "freq" and 14.0 < c[2] < 14.35 for c in flex.calls))


async def t5_stby_switch_fails():
    """Amp ignores the OPERATE toggle: FAIL, no tune."""
    orch, serial, flex, phases = make(op_status="Oper", oper_works=False)
    ok = await orch.tune_band("20m")
...
async def t6_stop_mid_sweep_restores_operate():
    ...
    check("t6 carrier off ran",
          flex.calls and ("carrier", False) in flex.calls)
...
async def t8_unknown_band():
    ...
    orch, serial, flex, phases = make(freq="9.500000")
...
async def t9_slice_unknown_trusts_request():
    """No slice data: explicit request trusted with a note (short wait)."""
    orch, serial, flex, phases = make(op_status="Stby")
    flex.slice_state = {}

    async def _no_populate():
        pass
    ok = await orch.tune_band("20m")
...
async def t11_digu_tunes_in_cw():
    """DIGU slice: mode flips to CW for the tune, DIGU restored after."""
    orch, serial, flex, phases = make(mode="DIGU")
    ok = await orch.tune_band("20m")
    check("t11 sweep succeeds", ok, str(phases))
    check("t11 MODE_SET emitted",
          any(p == "MODE_SET" and "CW" in m and "DIGU" in m
              for p, m in phases), str(phases))
    mode_calls = [c for c in flex.calls if c[0] == "mode"]
...
async def t12_cw_mode_untouched():
    """Slice already CW: no MODE_SET, no mode commands."""
    orch, serial, flex, phases = make(mode="CW")
    ok = await orch.tune_band("20m")
    ...
    check("t12 mode restore is CW (harmless)",
          all(c[2] == "CW" for c in flex.calls if c[0] == "mode"),
          str(flex.calls))


async def t10_flex_unreachable():
    orch, serial, flex, phases = make(op_status="Oper", flex_none=True)
    ok = await orch.tune_band("20m")
...
async def main():
    for t in (t1_sweep_operate_restore, t2_sweep_already_stby,
              ...
              t8_unknown_band, t9_slice_unknown_trusts_request,
              t10_flex_unreachable, t11_digu_tunes_in_cw,
