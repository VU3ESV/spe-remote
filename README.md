...
| `tune_band:<band>` | Sweep the SPE manual's recommended in-band sub-band centers. **The radio rules the band**: the rig's current freq is mapped to its ham band and that band is swept — the payload band (`160m`, `80m`, …, `6m`; also `auto` / `current` / empty) is overridden with a note if it disagrees, and only trusted when the rig's band can't be read. Auto-switches the amp to STBY, saves the operator's pre-sweep VFO freq + mode, hits each sub-band in turn, then restores the VFO and hands OPERATE back iff it was on at the start. |
...
```json
{"tune_event": "BAND_CHECKED",  "tune_message": "radio rules: on 20m (14.0740 MHz) — sweeping 20m"}
...
- **Operator picks the antenna; the radio picks the band.** Per the SPE manual's procedure the antenna is chosen with `[ANT]` before the sweep — spe-remote never touches antenna selection. The band, though, follows the radio: the slice freq is mapped to its ham band and *that* band is swept, overriding a stale menu pick with a note instead of refusing (the antenna on the amp follows the radio, so the radio's band is always the safe one to tune). The explicit pick is only trusted when the radio's band can't be read. STBY/OPERATE is handled for you — the amp is dropped to STBY for the sweep and OPERATE is restored afterwards iff it was on when the sweep started. The bundled web dashboard's band picker also auto-follows the amp's band (which tracks the radio via CAT).
- **Tunes in CW regardless of the operating mode.** The slice is switched to CW for the duration of the tune (and the original mode restored with the VFO afterwards): CW keys its carrier exactly on the dial freq, while DIGU/DIGL (and RTTY) carry a TX offset that, near a band edge, pushes the actual carrier out of band — where the Flex refuses to key and the cycle fails. Operator-observed on DIGU; automated from their own workaround ("switch to CW and tune").
...
| `tune_band:<band>` | Sweep the manual's sub-bands. The radio's current band rules — the payload band (e.g. `tune_band:20m`, or `auto`) is only trusted when the rig's band can't be read. Amp auto-STBY'd, OPERATE restored after |
